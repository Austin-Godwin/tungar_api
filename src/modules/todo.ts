...
export class Todo {
    ...
    static async create_many(todos: Array<Todo>) {

        let batch = fDb.batch();

        todos.forEach((todo, index) => {
            const id = index + todo.author;
            batch.set(todoColl.doc(id), { ...todo.toJson(), id },);
        });
    ...
    static fromJson(json: any) {
        const arg = new Todo(json.author, json.title, json.dueAt);
        return arg;
    }
