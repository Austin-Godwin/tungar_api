...
export default class User {
    ...
    static async generateToken(data: any) {
        return new Promise<string>((resolve, reject) => {
            jwt.sign(data, "temp", { algorithm: "HS512", expiresIn: "1d" }, (err, token) => {
    ...
    static async login(username: string, pwd: string) {
        ...
        try {
            ...
            const token = await User.generateToken(user.toJson());
    ...
    async register() {
        ...
        SendMail(this.email, "Welcome To Our Todo Application", `
        Hello!
                We are happy to have you join our platform. for support contact us at info@xyz.com
        `);
